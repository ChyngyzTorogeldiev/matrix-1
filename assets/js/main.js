...
const scrollUp = () =>{
  const scrollUp = document.getElementById('scroll-up')
    // When the scroll is higher then 350 viewport heght, add the
...
const sections = document.querySelectorAll('aection[id]')
...
const scrollActive = () =>{
    ...
    if(scrollDown > sectionTop && scrollDown <= sectionTop + sectionHeight){
      sectionClass.classList.remove.add('active-link')
    }else{
      sectionClass.classList.remove('active-link')
    }
  })
}
window.addEventListener('acroll'. scrollActive)
